...
void meshRender(
    ...
    for(int triNumber = 0; triNumber < mesh->triNum; triNumber += 1){
		...
		shadeVertex(sha->unifDim, unif, sha->attrDim, vertices[0], sha->varyDim, varyA);
		shadeVertex(sha->unifDim, unif, sha->attrDim, vertices[1], sha->varyDim, varyB);
		shadeVertex(sha->unifDim, unif, sha->attrDim, vertices[2], sha->varyDim, varyC);
