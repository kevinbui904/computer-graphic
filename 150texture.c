...
void texSample(const texTexture *tex, double s, double t, double sample[]) {
    ...
    if (tex->filtering == texNEAREST)
        ...
        double final[tex->texelDim];

        //get texel bottom left
        texGetTexel(tex, floor(u), floor(v), sample);
        vecScale(tex->texelDim, (1-fracU)*(1-fracV), sample, final);
        
        //get texel bottom right
        texGetTexel(tex, ceil(u), floor(v), sample);
        vecScale(tex->texelDim, fracU*(1-fracV), sample, sample);
        vecAdd(tex->texelDim, sample, final, final);

        //get texel top left
        texGetTexel(tex, floor(u), ceil(v), sample);
        vecScale(tex->texelDim, (1-fracU)*fracV, sample, sample);
        vecAdd(tex->texelDim, sample, final, final);

        //get texel top right
        texGetTexel(tex, ceil(u), ceil(v), sample);
        vecScale(tex->texelDim, fracU*fracV, sample, sample);
        vecAdd(tex->texelDim, sample, final, final);

        //copy final texel numbers into sample for returning
        vecCopy(tex->texelDim, final, sample);
    }
}
