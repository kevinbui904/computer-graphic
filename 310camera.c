...
void camGetProjectionInverseIsometry(const camCamera *cam, double homog[4][4]) {
	double proj[4][4], camInverseIsom[4][4], camIsom[4][4];
...
void camLookFrom(
	...
	//remove these after
	double test[4][4], test2[4][4], test3[4][4];

	isoGetHomogeneous(&(cam->isometry), test2);
	isoGetInverseHomogeneous(&(cam->isometry), test);
	mat444Multiply(test2, test, test3);
	mat44Print(test3);
}
