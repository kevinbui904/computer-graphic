...
void main() {
    ...
    iSpecDirectional = max(0, iDiffuseDirectional);
    iSpecPositional = max(0, iDiffusePositional); 

    rgbaTex = rgbaTex * body.cSpecular;
    //computing outColors
    outColor = pow(iSpecDirectional, 64.0f) * (scene.cLightDirectional * rgbaTex);
    outColor = outColor + (pow(iSpecPositional, 64.0f) * (scene.cLightPositional * rgbaTex));
